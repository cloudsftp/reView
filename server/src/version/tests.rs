...
fn parse_config_version_from_string() {
    ...
    for TestCase { input, expected } in [
        TestCase {
            ...
            expected: ConfigVersion::Ancient,
        },
        TestCase {
            input: FirmwareVersion {
                version: 2,
        ...
        let result = get_config_version(&input).expect("could not parse version");
...
fn parse_framebuffer_version_from_string() {
    ...
    for TestCase { input, expected } in [
        ...
        let result = parse_version(input).expect("could not parse version");
        assert_eq!(expected, result, "version input '{}'", input);
    }
}
